...
def check_booking(req: func.HttpRequest) -> func.HttpResponse:
    ...
    matching = [
        v for v in FLEET
        if str(v["type"]).lower() == vehicle_type
        and str(v["location"]).lower() == pickup_location.lower()
        and v["available"]
    ]
    ...
    if matching:
        ...
        type_matches = [v for v in FLEET if str(v["type"]).lower() == vehicle_type]
        location_matches = [v for v in type_matches if str(v["location"]).lower() == pickup_location.lower()]
        ...
        if not type_matches:
            ...
            if available_locations:
                reason = f"No {vehicle_type} available in {pickup_location}. Try: {', '.join(map(str, sorted(available_locations)))}"
            else:
